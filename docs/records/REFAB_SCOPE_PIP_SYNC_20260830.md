...
## 5.5 2026-08-30 两轮实机：硬件剪裁盒算不出来（已整体移除）

移植时本仓<b>自加</b>了一道保险：用目镜的「斜率空间包围盒」换算成屏幕 NDC，
给镜内合成开一个 scissor，让放大画面物理上不可能溢出镜片。姊妹分支没有这道东西。
两轮实机证明它在 26.2 上<b>无法被正确计算</b>，已整体移除。

### 第一轮：拿视图矩阵当投影（现象随朝向变化）

`GameRendererMixin` 在 `renderItemInHand` 的 HEAD 把<b>第三个参数</b>当投影矩阵
送给 `setHandProjection`。它在 26.2 上<b>不是投影，是视图矩阵</b>：

| | m00 | m11 | 随朝向变？ |
|---|---|---|---|
| 透视投影 | `1/(aspect·tan(fovY/2))`，恒正 | `1/tan(fovY/2)`，恒正 | 否 |
| 视图矩阵 | `∝ cos(yaw)` | `∝ cos(pitch)` | **是，并且变号** |

用户现象三条逐条对上：

1. **硬边界（整半球正常、另一半球异常）** —— 决定性证据。投影矩阵造不出这种边界；
   而 `hasMaskBounds()` 判据是 `projectionP00 > 0`，视图矩阵的 `m00` 在半个球面为负
   ⇒ 判据不成立 ⇒ **剪裁压根没开**，只剩软掩码 ⇒ 那半边看起来「正常」。
2. **尺寸随 yaw/pitch 缩小** —— 盒子 = `|cos(yaw)| · slope`，比真实孔径小。
   着色器里「掩码外 discard」是软约束，**只有盒子比孔径小时才看得见盒子**，
   看到的就是那个矩形（往东西偏长度变短、往上下偏宽度变窄、正南且视角平时最大）。
3. **「但不是很大」** —— 正南水平时盒子约为孔径的 1.25（横）× 0.70（纵），
   横向由孔径兜住、纵向被切掉 ⇒ 一个扁矩形。

### 第二轮：改用 CameraRenderState 的真投影（现象变成恒矩形）

按「`m33 == 0` 才是透视投影」判定后退回 `CameraRenderState#projectionMatrix`
（本仓已在用的真投影）。结果用户报：**变成恒矩形**。

这同样是一条决定性证据，而且指向反面：

- 盒子现在<b>恒定</b>（投影不再随朝向变）说明投影选对了；
- 盒子仍然<b>偏小</b>（还是矩形）说明 `P_hand > P_world`，即
  **手持那一遍用的 FOV 比世界那一遍窄** —— 两者本来就不是同一个矩阵。
  （世界 FOV 在 PIP 默认 `WorldZoomShare = 0` 下并不缩放，所以这不是缩放造成的，
   而是手持渲染自己用了更窄的固定 FOV。）
...
| 候选 | 实测结论 |
|---|---|
| `renderItemInHand` 第三参数 | 视图矩阵（随朝向变化、且变号） |
| `CameraRenderState#projectionMatrix` | 真投影，但是<b>世界</b>那一遍的，比手持宽 |
| `RenderSystem#getProjectionMatrix()` | 26.2 已不存在（本仓既有注释） |
| 投影 UBO 读回 | 有光影时必抛 `Buffer is not readable`（本仓既有日志，也是掩码改走斜率空间的原因） |

于是这道剪裁<b>无法被正确计算</b>：盒子偏小会切出矩形（有害），
而要保证「绝不偏小」就只能取得很宽，那时剪裁本身也没意义了。

### 处置

整体移除：`ScopeMaskRenderer` 的投影系数 / 包围盒状态 / 换算方法、
`ScopePipRenderer` 的 `applyLensScissor` 及其调用、`GameRendererMixin` 的矩阵采集与
两个判定方法全部删掉。合成现在只有着色器里的软掩码约束 —— <b>与姊妹分支一致</b>。

### 想把保险加回来，唯一正确的做法

不要再从 CPU 侧猜投影，改在<b>着色器里</b>做：合成 pass 本来就调了
`RenderSystem.bindDefaultUniforms(pass)`，环境 Projection uniform 就在那儿。
着色器取 `ProjectionMatrix[0][0]` / `[1][1]`，把 `texCoord` 反算回斜率空间，
与随 uniform 传进来的斜率包围盒比较后 discard —— 全程不需要 CPU 侧知道投影是多少。
需要落实的三件事：`Projection` 的 bind group layout 常量名、shader 侧
`#moj_import` 的 uniform 块名、以及那 4 个包围盒浮点的通道
（ModelView 矩阵的对角线是现成的空闲载体）。**未做**，等实机通道通了再说。
