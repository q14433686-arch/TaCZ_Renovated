...
| Controllable 编译坐标坑：CurseForge 文件页带 "Curse Maven does not yet support mods that have disabled 3rd party sharing"（Controllable 禁用第三方分享，CurseMaven 拿不到 7943194）；改走官方 GitHub Releases 签名 jar（配置期下载到 build/compat 缓存） | CurseForge 7943194 文件页警告行 + GitHub `MrCrayfish/Controllable` release `v0.26.0+26.1.2` 资产 `controllable-neoforge-0.26.0+26.1.2-signed.jar`（与 7943194 同版本同源 multiloader/26.1.2） |
