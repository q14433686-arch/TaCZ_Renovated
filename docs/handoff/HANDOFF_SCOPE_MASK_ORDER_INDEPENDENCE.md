...
- **上游纪元机制失效不要当 bug 修。** 你们同样是从 1.21.1 / 1.20.1 上游移植到 26.2。
  凡症状能指认到「只在上游纪元存在的机制」（1.21.1 还是 stencil 纪元、无 Feature
  Rendering；26.2 换了 `GlCommandEncoder` / `RenderPipeline` / FrameGraph 整套底座），
  在 26.2 下必然失效，属**预期结果**，不是移植遗漏。本仓已把这条写进
  `AGENTS.md` §3.8：归类为「不适用（上游纪元）」、写进状态文档、**不修不绕、
  不进 CHANGELOG 修复项**；确实需要该行为就按 26.2 机制重做并另开工作包。
  排查本任务时注意别把这类症状误当成本次要修的东西。
