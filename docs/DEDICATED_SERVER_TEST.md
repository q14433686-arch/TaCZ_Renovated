...
### B. 不对称安装（记录实际表现，不预设结论）

| 场景 | 预期方向 | 必须成立的底线 |
|---|---|---|
| 只装服务端 | 逻辑在、显示残（模型/贴图缺，名字可能显示原始翻译键——语言文件在客户端资产里） | 双端都**不崩**、不踢出 |
| 只装客户端 | 服务端不认包内 id，`/give` 报"未知物品"类错误 | 同上 |

### C. 在线热重载

双客户端在线时服务端执行 `/tacz reload`：新增/删除包后客户端内容随之刷新
（OnDatapackSyncEvent 全员重同步），无踢出、无解析错误。
