...
- [x] **光影下世界 GPU 路径可见性（用户报告后修复）**：`isRenderShadow` 空壳已按
      姊妹线同款反射修复（见 CHANGELOG 第三轮修正与 records 同步记录 §8）。
      **2026-09-02 维护者实机 PASS：开光影 + 默认开关下第三人称 / 掉落物 / 展示台
      高模枪恢复显示。**
- [ ] 光影下阴影贴图里枪影形状复核（cube-only 阴影是设计语义）；shell / muzzle 在
      光影下的表现复核。
