 ...
 * （{@code VoxyRenderSystem} 构造时就把 {@code pipeline}、
 * {@code viewportSelector}、遍历器全部绑到当时的那套 Iris 管线上）。
 ...
 *   <li>{@code VoxyRenderSystem.viewportSelector} —— 视口必须由<b>对应的</b> section renderer 产出；</li>
 *   <li>{@code HierarchicalOcclusionTraverser.pipeline} —— 遍历器每帧 {@code bindUniforms()}；</li>
 ...
 * 同步而来；唯一改动是 {@code FabricLoader#isModLoaded} → {@code ModList#isLoaded}。
 * 反射目标（{@code me.cortex.voxy.*}）与加载器无关。
 */
public final class VoxyScopePipelineCompat {
     ...
     * 查不出来时返回 {@code true}（当作已绑）：宁可不建，也不能冒险去撞那个异常。
     */
    ...
    public static boolean ensureBuilt(Object voxySystem) {
        ...
        try {
            ...
            // catch 住之后会调 IrisUtil.disableIrisShaders()，那会把整个 Iris 拆掉，
            // 紧接着主画面那一遍的 Voxy 绘制就 NPE 当场崩游戏。
            ...
            // shaderpack，编译结果等价 —— 需要的只是逐帧换绑 traversal.pipeline。
     ...
     * 而玩家一改区块视距，{@code LevelExtractor.allChanged()} 就会让 Voxy 走
     * {@code voxy$shutdownRenderer()} + {@code voxy$createRenderer()} —— 旧的
     * {@code VoxyRenderSystem.free()} 把 {@code traversal} 连同它那些 GlBuffer
     * 一起释放掉了。我们那第二套却还攥着<b>已经被释放</b>的旧 traverser，
     * 下次开镜时 {@code this.traversal.bind()} 就会撞上 {@code assertNotFreed}。
     */
    public static void onRendererRebuilt() {
        if (swapped) {
            // 理论不可达：这条路径由 allChanged() 驱动，而镜内那一遍期间 allChanged()
            // 会被整个取消掉。真发生了也不能在换上去的中途抽掉它 —— 留给 swapOut 收尾。
            return;
        }
        disposeScopeStack();
    }
    ...
    private static void disposeScopeStack() {
        ...
        if (pipeline != null) {
            try {
                ...
                // 少了这一步，下次为同一套 Iris 管线重建时会撞上 "Pipeline data already bound"。
