...
public class ProjectileExplosion extends ServerExplosion {
    ...
    public int explode() {
        ...
        // 26.2: getToBlow() 已移除，方块破坏列表由 ServerExplosion 内部管理
        ...
        for (Entity entity : entities) {
            ...
            if (entity instanceof LivingEntity livingEntity) {
                // 26.2: ProtectionEnchantment 已移除，使用 EnchantmentHelper.getDamageProtection 减免伤害
                float protection = EnchantmentHelper.getDamageProtection(this.level, livingEntity, this.getDamageSource());
                damage = damage * (1.0D - Math.min(protection, 20.0D) / 25.0D);
            }
