 ...
 * {@code restoreFrom}（含背包恢复）与 {@code initInventoryMenu} 之后触发
 * （本线证据：NeoForge 1.21.11 分支
 * {@code patches/net/minecraft/server/players/PlayerList.java.patch} →
 * {@code EventHooks.firePlayerRespawnEvent(serverplayer, p_11238_)}），
 * {@code event.getEntity()} 即为重生后的新玩家实例；上游 Fabric 侧的
 * {@code alive} 参数（对应此处 {@code isEndConquered()}）在原实现中即未使用，
 * 保持不过滤。枪械状态机本身的重置由 {@code ServerPlayerMixin#restoreFrom}
 * 负责，与本类无关。</p>
 */
