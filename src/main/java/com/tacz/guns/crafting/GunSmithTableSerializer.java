...
public final class GunSmithTableSerializer {
     ...
     * {@link GunSmithTableIngredient#GunSmithTableIngredient(com.google.gson.JsonElement, int)} 延迟解析。
     *
     * <h2>为什么不能直接 {@code Ingredient.CODEC.fieldOf("item")}（refab 26.3 §4.3 实机定案）</h2>
     * <p>枪包里大量材料仍是旧写法：{@code {"tag":"c:ingots/iron"}}、
     * {@code {"type":"tacz:nbt","nbt":{...},"partial":true,"items":"tacz:attachment"}}。
     * 这些写法只有 {@link GunSmithTableIngredient#resolve} 里的 {@code normalizeLegacyIngredient}
     * 会改写成 26.3 认得的形态；Gson 路径（供 GUI/JEI）一直走那里，所以界面正常。
     * 但本 codec 是<b>配方注册表</b>加载路径（26.3 {@code RegistryDataLoader} 把 recipe 当动态注册表加载），
     * 此前直接把原文喂给 {@code Ingredient.CODEC} → 动态注册表任一元素解析失败 ⇒
     * <b>整个存档进不去</b>（只装默认枪包时恰好没有旧写法材料，所以此前没暴露）。</p>
     *
     * <p>{@code ExtraCodecs.JSON} 把任意值原样转成 {@code JsonElement}，后续解析、失败日志、
     * 判空语义全部与 Gson 路径统一；编码方向仍写出解析后的 Ingredient。</p>
     */
