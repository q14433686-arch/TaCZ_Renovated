...
public final class ScopeDepthCopyState {
     ...
     * {@link #hasMaskCycleThisFrame()} 查询「周期是否落在当前帧」。
     */
     ...
     * 于枪身绘制）、Iris 终局钩子处的 PIP 合成。没有当帧周期的帧（腰射，或当帧周期被
     * 身份守卫否决）闸门为假：前者不拿陈旧镜孔裁枪身（实机 2026-09-01 的静态透视面病例），
     * 后者不拿遗留拷贝贴镜内画面（同日的一帧「截图」贴屏病例）—— 都 fail-closed。</p>
     */
