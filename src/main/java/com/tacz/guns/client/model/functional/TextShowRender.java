...
import com.tacz.guns.client.render.scope.ScopeTextSubmitter;
...
public class TextShowRender implements IFunctionalSubmitter {
    ...
    /**
     * 是否把文字裁进目镜掩码。挂在瞄具 ocular 子树上的文字（弹药计数等）传 true：flush 时走
     * {@link ScopeTextSubmitter} 的孔径深度掩码管线，溢出圆孔的像素被丢弃；枪身文字保持 false ——
     * vanilla 管线按场景内容正常处理。
     */
    ...
    public void extract(ExtractionContext context) {
        ...
        if (clipToScopeMask) {
            context.add(collector -> {
                if (!ScopeTextSubmitter.submit(collector, taskPose, -xOffset, -font.lineHeight / 2f,
                        sequence, shadow, packedLight, color)) {
