...
public final class PolyMeshGpuRenderer {
    ...
    public static boolean isGpuPathUsable() {
        ...
        // entityCutout 的 RenderPipeline —— Iris 26.2 在初始化时自动将该管线归入
        // HAND program（抛壳/火光同一条兼容链路），枪体因此拿到光影光照。
        // 顶点常驻 VBO 不变，每帧仍只写 O(骨骼) 个 DynamicTransforms。
     ...
     * ENTITY_CUTOUT 提交，Iris 26.2 自动将该管线归入 HAND program，光影下渲染正确）。
     * 这里用 {@code RenderTypes.entityCutout} —— 管线正是 ENTITY_CUTOUT，Iris 对它的
     * 接管方式与 vanilla 立方体/collector poly 完全一致。
     *
     ...
     *
     * <p>与手部变体唯一的语义差异：世界 pass 里 ENTITY_CUTOUT 就是 vanilla 世界
     * 实体在用的管线，Iris 对它的默认接管（gbuffers_entities 链路）正是我们想要的。
    ...
    private static long drawViaRenderTypeCore(List<DrawEntry> draws, boolean handPass,
        ...
        //                iris_NormalMat（26.3 的 ExtendedShader 与 26.2 逐字相同，
        //                法线病灶依旧存在）。
        ...
        // 深度必须留着，GPU poly 才能被正确遮挡）。
        // 【26.3】externalPass != null 时直接复用调用方那个 pass。
        // 26.3 把 pass 的归属倒置：世界/手部的绘制都发生在 vanilla 已经开好的
        // pass 内部（LevelRenderer 的 "Solid"、GameRenderer 的 "Item in hand"），
        // 此时再 createRenderPass 会撞
        // "Close the existing render pass before creating a new one!"。
        // 复用而非另开，附件自然与 vanilla 一致（就是主 target 的颜色+深度）。
        // 【法线 · 每骨骼强制重 bind】见 IrisGlCommandEncoderMixin：Iris 只在管线
        // 切换时重算 iris_NormalMat，同管线连续的骨骼会沿用第一根的法线矩阵。
        // 绘制期间置位，让每次 draw 都走 pipeline.bind() → Iris setupState 读到
        // 本骨骼的 MV 栈顶。
     ...
     *
     * <p>从原 {@code drawList} 的 try(pass) 块原样抽出 —— 抽出的唯一目的，是让
     * 「自己开 pass」与「复用 vanilla 的 pass」两条路共享同一段绘制逻辑。
     * 26.3 起后者是常态（见 drawList 里的 externalPass 分支）。</p>
     */
