...
public enum LrClientAssetsManager {
     ...
     */
    public void reloadAndRegister(java.util.function.BiConsumer<Identifier, net.minecraft.server.packs.resources.PreparableReloadListener> register) {
        if (throwableDisplay == null) {
            throwableDisplay = new ThrowableDisplayManager(GSON);
            meleeDisplay = new MeleeDisplayManager(GSON);
        }
        register.accept(throwableDisplay.ID, throwableDisplay);
        register.accept(meleeDisplay.ID, meleeDisplay);
    }
