 ...
 * 26.2 {@code Minecraft#handleKeybinds}（refab 来源分支对 26.2 jar 字节码逐条核对，
 * 偏移 657-687；26.1.2 属同一代引擎，行为一致，本仓未再持 jar 复核）：
 * <pre>
 ...
 * {@code MultiPlayerGameMode#useItem}（refab 对 26.2 字节码核对）把
 ...
 *
 * <h2>时序为什么可靠（不是碰运气）</h2>
 * 26.2 {@code Minecraft#tick} 的调用顺序（refab 字节码偏移）：
 ...
 * {@code ClientTickEvent.Post}（NeoForge 的 {@code Minecraft#tick} 末尾，见
 * {@code LrClientEvents#onClientTickPost}）时，一定能在<b>同一次</b> tick 里看到
 * 下降沿，而下一次 {@code handleKeybinds} 已经是下一个 tick —— 拦得住，
 * 不存在「慢一帧」的窗口。
 *
 * <h2>刻意收窄的范围</h2>
 * <ul>
...
public final class UsePressGate {
    ...
    /**
     * 每客户端 tick 末尾调用（NeoForge {@code ClientTickEvent.Post}，见
     * {@code LrClientEvents#onClientTickPost}）。
     *
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        if (player.isUsingItem()) {
            ...
            //      Predicate（refab jar 内核对：is(Ljava/util/function/Predicate;)Z，
