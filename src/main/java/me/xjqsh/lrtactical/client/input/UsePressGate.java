 ...
 * {@code Minecraft#handleKeybinds}（本地 jar 字节码逐条核对）：
 * <pre>
 * while (options.keyUse.isDown()) {    // 右键按住期间每 tick 都进
 *     ...
 *     if (rightClickDelay == 0)
 *         if (!player.isUsingItem())
 *             startUseItem();           // ← 只要「没在使用中」就重新开始
 * }
 * </pre>
 ...
 *   <li>投掷物：{@code ThrowableItem#getUseDuration} 返回 72000（一小时），
 ...
 * {@code MultiPlayerGameMode#useItem} 把 {@code ServerboundUseItemPacket} 放在
 * {@code startPrediction} 的回调里构造，<b>先于</b> {@code ItemStack#use} 调用 ——
 * 所以只在 {@code use} 里返回 FAIL 拦不住包：服务端照样会被问一次，而服务端没有
 * 「这次按压已经用过了」的概念，于是服务端 {@code startUsingItem}、客户端没有 →
 * 换一个方向的分叉。拦在 {@code startUseItem} 之前则两头都干净：
 * 本地不进入使用状态，包也压根不发。
 *
 * <h2>时序为什么可靠（不是碰运气）</h2>
 * {@code Minecraft#tick} 里<b>输入处理先于实体/世界 tick</b>，而「使用结束」发生在
 * 实体 tick 里（服务端停用时靠同步的 {@code DATA_LIVING_ENTITY_FLAGS} 落地）。
 * 本类挂在 NeoForge 的 {@code ClientTickEvent.Post}（{@code Minecraft#tick} 末尾）时，
...
public final class UsePressGate {
    ...
    /**
     * 每客户端 tick 末尾调用（{@code ClientTickEvent.Post}）。
     *
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        if (player.isUsingItem()) {
            ...
            //      比较用 getItem() == 而不是 ItemStack#is(...)：不同 MC 版本 is 的
            //      重载集合不一致（1.21.x 只有 Predicate 版），直接比 Item 引用
            //      最省事也最不容易踩版本差异。
