 ...
 * 26.2 本内置层   : WeakHashMap
 *                    ModCapabilities.coolDowns(player).tick()
 * </pre>
 *
 * <h2>为什么保留 WeakHashMap</h2>
 * 26.1.2 R1 已用这套实现完成单机与专服验收；状态只包含瞬时冷却/连招，不需要持久化。
 * 26.2 前滚继续沿用该稳定语义，而不是仅为换用 attachment 引入新的生命周期变量。
 * TaCZ 侧 {@code DataHolderCapabilityProvider} 也使用 WeakHashMap，保持全仓一致。
 *
...
public final class ModCapabilities {
    ...
    public static void init() {
        // WP-LR2：refab 自建 PlayerTickEvent.START → NeoForge 原生 PlayerTickEvent.Pre
        // （WP07 C 映射表已验证的对应；双端玩家均触发，配合按端分表各 tick 一次）。
        ...
        // WP07 C 映射表：ServerPlayerEvents.AFTER_RESPAWN → PlayerEvent.PlayerRespawnEvent
        // （NeoForge 事件无旧玩家引用——见 onRespawn 的 WP-LR2 适配说明）。
     ...
     */
    // WP-LR2 适配：NeoForge PlayerRespawnEvent 不提供旧玩家引用（WP07 C 映射表明示）。
    // 本类 map 的 key 相等性按 Entity#equals = 网络 id（见上方分析），而
    // PlayerList#respawn 让新玩家沿用旧 id —— 因此 remove(newPlayer) 必命中
    // 旧玩家留下的同一条目，语义与 refab 的双重 remove 等价。
    private static void onRespawn(net.minecraft.server.level.ServerPlayer newPlayer) {
    ...
    /**
     * 客户端侧的同一问题 —— 由 {@code LrClientEvents} 每 tick 调用。
     *
     * <h2>为什么这里用引用轮询</h2>
     ...
     */
    // WP-LR2：@Environment 系注解在 NeoForge 上一律移除（本仓纪律：注解不是保护）。仅客户端调用。
    public static void onClientPlayerTick(Player current) {
        java.lang.ref.WeakReference<Player> ref = lastClientPlayer;
        Player previous = ref == null ? null : ref.get();
        if (current == previous) {
            return;
        }
     ...
     */
    private static java.lang.ref.WeakReference<Player> lastClientPlayer;
