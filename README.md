...
本仓库把 TaCZ 移植到 **Minecraft 1.21.11 NeoForge**（1.21.11 线，由 26.1.2 线
回移植而来）。游戏语义来自姊妹项目
[TaCZ Refabricated Unofficial](https://github.com/q14433686-arch/TaCZ_Refabricated_Unofficial)
（Fabric 26.x / 1.21.11 移植）的 1.21.11 分支——该分支已把同一条回移路线完整走完，
本移植消费其全部踩坑台账（见 [`docs/PORT_12111_BRIEF.md`](docs/PORT_12111_BRIEF.md)）；
本仓库当前源码版本为 **`1.1.8+neoforge.1.21.11.R0`**。
...
| Minecraft | 加载器 | 状态 |
|---|---|---|
| **1.21.11** | NeoForge | **本仓库**（回移植线，当前源码版本见上方；Releases 发布前请从源码构建） |
| **26.1.2** | NeoForge | 本仓库 26.1.2 分支（维护线，R1 已发布） |
...
（1.21.11 线的兼容矩阵随回移工作包逐项重钉，当前以该文档实际标注为准。）
...
- TaCZ 的 NeoForge 1.21.11 端口（26.1.2 线 R1 代码回移植）及随上游带来的默认枪包；
- 为 1.21.x API 与 NeoForge 事件面改写的注册、网络、资源加载、GUI 和渲染接线；
- 若干可选模组的兼容接线。

**LRTactical 内置框架**：throwable/melee/detonator/consumable 四类物品与
...
1.21.11 线沿用 26.1.2 线的 **depth-aperture（深度孔径）** 方案：在绘制边界备份/恢复
深度并做目镜孔径拷贝，镜片后看到的仍是同一次世界渲染——没有第二台相机，没有第二次
`renderLevel`。1.21.11 的管线 API 不同（`RenderPipeline` 回归 `RenderType`/
`RenderStateShard`，`CompareOp.ALWAYS_PASS` 无直接等价物），回移方案与验收点见
[`docs/PORT_12111_BRIEF.md`](docs/PORT_12111_BRIEF.md) §4-E。
代码中的 `PictureInPictureRenderer` **仅用于枪械工作台的 GUI 模型预览**，与瞄具无关。

装有 Iris 时的 HAND shader 接线随回移工作包推进，落地前以
[docs/COMPATIBILITY.md](docs/COMPATIBILITY.md) 的实际标注为准；
...
- **联机**：局域网与真实专用服务器在 **26.1.2 线 R1** 均已实测通过（2026-08-21）；
  **1.21.11 线待回移完成后重新走同套实测**（测试预案与形态矩阵见
  [docs/DEDICATED_SERVER_TEST.md](docs/DEDICATED_SERVER_TEST.md)，验收纪律不得降级）。
...
需要 JDK 21（构建会拉取官方映射 + Parchment + NeoForge 21.11.x 工件）：
